...
const SPECIAL = ITEMS.find((it) => it.type === ITEM_TYPES.SPECIAL);
const BAD = ITEMS.find((it) => it.type === ITEM_TYPES.BAD);
...
function HowTab() {
  return (
    <div className="space-y-3 text-[13px] leading-relaxed text-white/70">
      <p className="rounded-xl bg-moon-500/10 px-3.5 py-3 text-[13px] leading-relaxed text-moon-300 ring-1 ring-moon-500/25">
        떨어지는 <b>좋은 재료는 터치</b>하고, <b>방해 아이템은 그냥 두세요.</b>
        <br />
        <b className="text-moon-100">시간 제한은 없습니다.</b> 놓치지만 않으면 계속됩니다.
      </p>

      <Section title="🎯 이렇게 하면 돼요">
        <li>위에서 떨어지는 재료를 손가락으로 <b className="text-moon-300">톡톡 터치</b>하세요.</li>
        <li>재료(🥟🍚🌰🌿)는 <b className="text-moon-300">+100점</b>, 보름달(🌕)은 <b className="text-moon-300">+300점</b>입니다.</li>
        <li>터치하지 않은 재료는 바닥에 닿으면 사라집니다.</li>
      </Section>

      <Section title="💀 이러면 게임이 끝나요">
        <li>
          좋은 재료를 <b className="text-hanbok">{MAX_MISS}개 놓치면</b> 그 자리에서 끝납니다.
        </li>
        <li>화면 위쪽의 점 {MAX_MISS}개가 <b className="text-white/85">남은 기회</b>입니다.</li>
        <li>방해 아이템(🔥🥀)은 <b className="text-white/85">놓쳐도 줄지 않습니다.</b> 그냥 피하세요.</li>
      </Section>

      <Section title="📈 오래 버틸수록 레벨이 올라가요">
        <li>
          <b className="text-moon-300">{LEVEL_DURATION}초마다 레벨 +1</b> 입니다. 레벨 1부터 시작합니다.
        </li>
        <li>레벨이 오르면 <b className="text-moon-300">더 빠르게, 더 많이</b> 쏟아집니다.</li>
        <li>그래서 오래 버틸수록 점수를 크게 모을 수 있습니다.</li>
      </Section>

      <Section title="⭐ 그룹장 K 를 노리세요">
        <li>
          한 번 터치에 <b className="text-moon-300">+{won(SPECIAL.points)}점</b>, 가장 높은 점수입니다.
        </li>
        <li>
          <b className="text-moon-300">{chanceOf(SPECIAL).toFixed(0)}% 확률</b>로만 나오는 귀한 아이템입니다.
        </li>
        <li>
          단, <b className="text-hanbok">양옆에 방해 아이템 {ESCORT_COUNT}개</b>를 달고 내려옵니다.
          <b className="text-white/85"> 가운데만</b> 정확히 누르세요.
        </li>
      </Section>

      <Section title="🔥 연속으로 모으면 배수가 붙어요">
        <li>
          좋은 재료를 <b className="text-moon-300">{COMBO_STEP}개 연속</b>으로 모을 때마다 배수가 +1 됩니다
          (최대 <b className="text-moon-300">×{MAX_MULTIPLIER}</b>).
        </li>
        <li>
          배수가 곱해지므로 그룹장 K 는 최대{" "}
          <b className="text-moon-300">{won(SPECIAL.points * MAX_MULTIPLIER)}점</b>까지 오릅니다.
        </li>
        <li>
          방해 아이템을 터치하거나 재료를 놓치면 <b className="text-hanbok">배수는 처음으로</b> 돌아갑니다.
        </li>
      </Section>

      <Section title="😵 방해 아이템은 건드리지 마세요">
        <li>
          터치하면 <b className="text-hanbok">{won(BAD.points)}점</b>, 게다가{" "}
          <b className="text-hanbok">약 {(STUN_MS / 1000).toFixed(1)}초 동안</b> 아무것도 누를 수 없습니다.
        </li>
        <li>그래도 점수가 <b className="text-white/85">0점 아래로 내려가지는 않습니다.</b></li>
      </Section>

      <Section title="🏆 기록 남기기">
        <li>게임이 끝나면 이름을 넣고 점수를 등록합니다.</li>
        <li>이름은 <b className="text-moon-300">공백 없이 한글·영문·숫자 1~12자</b>.</li>
        <li>같은 이름의 <b className="text-moon-300">최고 점수 하나만</b> 랭킹에 올라갑니다.</li>
      </Section>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="rounded-xl bg-moon-500/10 px-3 py-2.5 text-[11px] leading-relaxed text-moon-300 ring-1 ring-moon-500/25">
        ...
        <b>{won(SPECIAL.points * MAX_MULTIPLIER)}점</b>까지 오릅니다.
      </p>
    </div>
  );
}
