...
떨어지는 송편 재료를 터치해 고득점에 도전하는 **모바일 전용** 이벤트 웹게임입니다.
시간 제한 없이 레벨이 계속 올라가며, 재료를 10개 놓치면 끝납니다.
...
2. **SQL Editor** 에 `supabase/schema.sql` 내용을 붙여넣고 실행
   (이미 운영 중이라면 참여 시간 설정용 `event_config` 테이블이 추가됐으므로 다시 한 번 실행해 주세요.
   전부 `if not exists` 라 기존 기록은 그대로 유지됩니다.)
...
관리자가 **참여 시간(시작·종료)** 을 설정해 두면 그 시간 밖에서 `게임 시작하기` 를 눌렀을 때
`이벤트 참여시간이 종료되었습니다.` (시작 전이면 `아직 이벤트 참여시간이 아닙니다.`) 안내를 보여주고
첫 화면에 그대로 머무릅니다. `/game` 주소로 바로 들어와도 같은 안내 후 첫 화면으로 보냅니다.

### 플레이 흐름

```
첫 화면(랭킹보드) → [게임 시작하기] → 3·2·1 카운트다운
   → 레벨을 올리며 플레이 (재료 10개 놓치면 종료) → 게임 오버 오버레이
...
- 이름을 바꿔 등록하면 **변경 전 이름으로 저장돼 있던 기록도 새 이름으로 함께 옮겨집니다.**
  새 이름에 이미 기록이 있으면 둘 중 **높은 점수만** 남기고 옛 기록은 지웁니다.
  기기를 넘겨받은 다른 사람이라면 결과 화면의 `새 참가자로 등록하기` 를 눌러 이관하지 않을 수 있습니다.
...
- **시간 제한이 없습니다.** 10초마다 레벨이 1씩 올라가고, 레벨이 오를수록 더 빠르고 더 자주 쏟아집니다.
- **감점 없는 재료를 10개 놓치면 종료**됩니다. 방해 아이템은 놓쳐도 기회가 줄지 않습니다.
...
app/
  ...
  actions.js           Server Actions (조회 · 점수 등록 · 관리자 · 참여 시간)
...
lib/
  ...
  eventWindow.js       참여 시간 판정·표시 헬퍼 (서버/클라이언트 공용)
supabase/
  schema.sql           테이블(scores · event_config) · 인덱스 · RLS 정책
```

## 7. 관리자 패널

첫 화면의 **`🌕 달토끼 타임어택` 배지**를 누르면 비밀번호를 묻습니다.
`PASSWORD` 환경변수와 일치하면 바텀시트가 열리고, 틀리면 안내 후 **첫 화면으로 돌아갑니다.**

| 기능 | 설명 |
|---|---|
| 전체 기록 초기화 | 두 번 눌러야 실행됩니다 |
| 이름으로 기록 삭제 | 대소문자 무시 |
| 게임 참여 시간 | 시작·종료 일시 설정 (`event_config` 테이블에 저장) |
| 전체 순위 다운로드 | 등수·이름·점수·등록일시 CSV (Excel 대응 BOM 포함) |

참여 시간은 브라우저 로컬 시간으로 입력받아 **UTC 로 저장**하고, 화면에는 다시 보는 사람의 로컬 시간으로 보여줍니다.
한쪽을 비워 두면 그 방향으로는 제한하지 않고, 둘 다 비우면 언제든 참여할 수 있습니다.
설정이 없거나 조회에 실패하면 **제한 없음**으로 처리해 게임을 막지 않습니다.

비밀번호는 **서버에서만 검증**하며, 네 기능 모두 호출할 때마다 다시 검증합니다.
